...
PanImFeatures <- readr::read_rds(path="/extraspace/yye1/analysis/2019SexImm/code/CodeSummary/Data/PanImFeatureFrom_PMID29628290.rds.gz")
...
for(cancer in cancerNames){
  ...
  # perform calculation
  # Load Aneuploidy data and processing
  EachImmune <- PanImFeatures[PanImFeatures$cancer_types==cancer,]$ImmuneFeatures[[1]]
  EachImmune <- EachImmune[!duplicated( EachImmune$TCGA.Participant.Barcode),]
  EachImmune.pri <- EachImmune[,c("Aneuploidy.Score")]
  EachImmune.pri <- as.matrix(EachImmune[,"Aneuploidy.Score"])
  colnames(EachImmune.pri) <-"Aneuploidy.Score"
  rownames(EachImmune.pri) <- substr(EachImmune$TCGA.Participant.Barcode,1,12)
  EachImmune.pri <- rm.zero.col(EachImmune.pri)
  
  ###Run the Propensity score algorithm
  ...
  if(length(which(EachImmune.result$pvalue < 0.05)) > 0){
    ...
    if(nrow(sum.AneuploidyAll) == 0){
      sum.AneuploidyAll<- sum.Immune
    }else{
      sum.AneuploidyAll<- rbind(sum.AneuploidyAll,sum.Immune)
    }
    
  }
}

write.table(sum.AneuploidyAll,file="Aneuploidy Score.across.cancer.typesAll.txt",quote = F,sep="\t",row.names = F)
