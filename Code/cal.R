...
{  #molecular.pri <- mutation.pri[,keep.index]
  ...
  write.table(t(rbind(rownames(clinical.common),ifelse(clinical.common$Z==1, "FEMALE","MALE"))), file=paste(cancer,"_",data.type,"_samples.txt", sep=""),quote = F,row.names = F,sep="\t")
  ...
  ##loading PSM
  source("/extraspace/yye1/analysis/2019SexImm/code/CodeSummary/GIPW_function_omega.R")
  ...
  wt <- ans$W
  source("/extraspace/yye1/analysis/2019SexImm/code/CodeSummary/check_balance.R")
