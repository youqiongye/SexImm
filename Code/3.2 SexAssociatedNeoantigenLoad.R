...
for(cancer in cancerNames){
  ...
  # perform calculation
  # Load neoantigen data and processing
  ...
  EachImmune.pri <- EachImmune[,c("SNV.Neoantigens")]
  EachImmune.pri <- as.matrix(EachImmune[,"SNV.Neoantigens"])
  colnames(EachImmune.pri) <-"SNV.Neoantigens"
  ...
  if(length(which(EachImmune.result$pvalue < 0.05)) > 0){
    ...
    if(nrow(sum.NeoantigenAll) == 0){
      sum.NeoantigenAll<- sum.Immune
    }else{
      sum.NeoantigenAll<- rbind(sum.NeoantigenAll,sum.Immune)
    }
    
  }
}

write.table(sum.NeoantigenAll,file="Neoantigen.across.cancer.typesAll.txt",quote = F,sep="\t",row.names = F)
