...
for(cancer in cancerNames){
  ...
  # perform calculation
  # Load TCR_BCR data and processing
  ...
  EachImmune.pri <- EachImmune[,c("TCR.Richness","BCR.Richness")]
  EachImmune.pri <- as.matrix(EachImmune[,c("TCR.Richness","BCR.Richness")])
  colnames(EachImmune.pri) <-c("TCR.Richness","BCR.Richness")
  ...
  if(length(which(EachImmune.result$pvalue < 0.05)) > 0){
    ...
    if(nrow(sum.TCR_BCRAll) == 0){
      sum.TCR_BCRAll<- sum.Immune
    }else{
      sum.TCR_BCRAll<- rbind(sum.TCR_BCRAll,sum.Immune)
    }
    
  }
}

write.table(sum.TCR_BCRAll,file="TCR_BCR.across.cancer.typesAll.txt",quote = F,sep="\t",row.names = F)
