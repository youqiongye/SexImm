...
setwd(scripts.dir)
###Loading the pan cancer mutation burden data
mutationBurdenT <- readr::read_rds("/extraspace/yye1/analysis/2019SexImm/code/CodeSummary/Data/PanCancerTMB.rds.gz")
...
analysis="gender" 
sum.PanTMB.All <- data.frame()
####must exist stum  clinical, stratification, Immune files.
...
for(cancer in cancerNames){
  ...
  # perform calculation
  # ImmuneCells
  EachImmune <- mutationBurdenT[mutationBurdenT$cancer_types==cancer,]$ImmuneFeatures[[1]]
  EachImmune <- EachImmune[!duplicated( EachImmune$barcode),]
  EachImmune.pri <- as.matrix(EachImmune[,"mutBurden"])
  colnames(EachImmune.pri) <-"mutBurden"
  rownames(EachImmune.pri) <- substr(EachImmune$barcode,1,12)
  ...
  write.table(as.matrix(t(unlist(EachImmune.result))), file=paste(cancer, analysis,"Immune.txt",sep="_"),quote=F,row.names=F,sep="\t")
  ...
  if(length(which(EachImmune.result$pvalue < 0.05)) > 0){
    ...
    if(nrow(sum.PanTMB.All) == 0){
      sum.PanTMB.All <- sum.Immune
    }else{
      sum.PanTMB.All <- rbind(sum.PanTMB.All,sum.Immune)
    }
  }
}


write.table(sum.PanTMB.All,file="MutationBurden.across.cancer.typesAll.txt",quote = F,sep="\t",row.names = F)
