...
###Loading the pan cancer mRNA expression of immune checkpoint
ImmCP <- readr::read_rds("/extraspace/yye1/analysis/2019SexImm/code/CodeSummary/Data/PanCancerImmuneCheckpointExpression.rds")

analysis="gender" 
sum.ImmCPAll <- data.frame()
...
for(cancer in cancerNames){
  ...
  # ImmuneCells
  CP <- ImmCP[ImmCP$cancer_types==cancer,]$InAcMarkerExp[[1]]
  CP <- CP[substr(CP$barcode,14,15) %in% c("01"),] %>% dplyr::mutate(barcode=substr(barcode,1,12))
  CP <- CP[!duplicated( CP$barcode),]
  CP.pri <- CP[2:ncol(CP)]
  colnames(CP.pri) <- colnames(CP)[2:ncol(CP)]
  rownames(CP.pri) <- CP$barcode
  CP.pri <- rm.zero.col(CP.pri)
  CP.pri <- apply(CP.pri,2,function(x){log2(x+1)})
  folder <- paste(cancer,"_",analysis,sep="")
  if (!file.exists(folder)) { dir.create(folder) }
  
  CP.result <- weight.test(data.dum, form, CP.pri, is.continuous=TRUE,weight=ifelse(analysis=="gender","ATT","MW"),mirror.plot=FALSE, cancer, data.type= "CP", outdir=paste(scripts.dir, "/",cancer,"_",analysis,sep=""),perm=FALSE)
  
  sum.Immune <- summarize.p(CP.pri, CP.result, print=TRUE)
  summarize.p(CP.pri, CP.result, print=TRUE, cutoff=0.05)
  write.summary(sum.Immune, cancer, analysis,"Immune")
  write.result(CP.result, cancer, analysis,"Immune")
  save(CP.result, file=paste(cancer,"_", analysis,"_result.RData",sep=""))
  if(length(which(CP.result$pvalue < 0.05)) > 0){
    sum.Immune <- data.frame(sum.Immune)
    sum.Immune$class <- rep(cancer,times=nrow(sum.Immune))
    if(nrow(sum.ImmCPAll) == 0){
      sum.ImmCPAll <- sum.Immune
    }else{
      sum.ImmCPAll <- rbind(sum.ImmCPAll,sum.Immune)
    }
   
  }
}

write.table(sum.ImmCPAll,file="CP difference.across.cancer.typesAll.txt",quote = F,sep="\t",row.names = F)
