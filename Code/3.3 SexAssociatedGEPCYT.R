...
###Loading the pan cancer GEP and CYT data
GEP_CYT <- readr::read_rds(path="/extraspace/yye1/analysis/2019SexImm/code/CodeSummary/Data/PanCancerGEP_CYT_Data.rds.gz")


analysis="gender" 
sum.GEPAll <- data.frame()
...
for(cancer in cancerNames){
  ...
  # ImmuneCells
  EachImmune <- GEP_CYT[GEP_CYT$cancer_types==cancer,]$GEPscore[[1]]
  ...
  EachImmune.pri <- EachImmune[,c("GEP","CYT")]
  colnames(EachImmune.pri) <- c("GEP","CYT")
  ...
  if(length(which(EachImmune.result$pvalue < 0.05)) > 0){
    ...
    if(nrow(sum.GEPAll) == 0){
      sum.GEPAll <- sum.Immune
    }else{
      sum.GEPAll <- rbind(sum.GEPAll,sum.Immune)
    }
    
  }
}

write.table(sum.GEPAll,file="GEP_CYT.across.cancer.typesAll.txt",quote = F,sep="\t",row.names = F)
